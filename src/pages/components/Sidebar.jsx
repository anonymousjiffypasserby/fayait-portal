...
export default function Sidebar({ activeView, onViewChange, components }) {
  const countFor = (view) => {
    if (view.key === 'all')       return components.length
    if (view.key === 'available') return components.filter(c => !c.retired && availableQty(c) > 0).length
    if (view.key === 'installed') return components.filter(c => parseInt(c.qty_installed || 0) > 0).length
    if (view.key === 'lowstock')  return components.filter(isLowStock).length
    if (view.key === 'retired')   return null
    return null
  }

  const totalQty     = components.reduce((s, c) => s + (c.quantity || 0), 0)
  const installedQty = components.reduce((s, c) => s + parseInt(c.qty_installed || 0), 0)
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', gap: 6, marginBottom: 12, padding: '0 2px' }}>
        ...
        <div style={{ flex: 1, background: '#fff', borderRadius: 8, padding: '8px 10px', border: `1px solid ${T.border}` }}>
          <div style={{ fontSize: 9, color: T.muted, textTransform: 'uppercase', letterSpacing: 0.8, marginBottom: 2 }}>Installed</div>
          <div style={{ fontSize: 18, fontWeight: 700, color: T.blue }}>{installedQty}</div>
        </div>
      </div>
