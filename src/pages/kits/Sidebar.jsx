...
const VIEWS = [
  { key: 'all',         label: 'All Kits',       icon: '◫' },
  { key: 'checkedout',  label: 'Checked Out',     icon: '↗' },
  { key: 'available',   label: 'Available',       icon: '◯' },
]

export default function Sidebar({ activeView, onViewChange, kits }) {
  const countFor = (view) => {
    if (view.key === 'all')        return kits.length
    if (view.key === 'checkedout') return kits.filter(isCheckedOut).length
    if (view.key === 'available')  return kits.filter(k => !isCheckedOut(k)).length
    return null
  }
  ...
  const checkedOut = kits.filter(isCheckedOut).length
  const available  = kits.length - checkedOut

  return (
    <div style={{
      width: 200, flexShrink: 0, borderRight: `1px solid ${T.border}`,
      padding: '12px 8px 24px', background: '#fafbfc',
      minHeight: 'calc(100vh - 56px)', overflowY: 'auto', boxSizing: 'border-box',
    }}>
      {/* Quick stats */}
      <div style={{ display: 'flex', gap: 6, marginBottom: 12, padding: '0 2px' }}>
        <div style={{ flex: 1, background: '#fff', borderRadius: 8, padding: '8px 10px', border: `1px solid ${T.border}` }}>
          <div style={{ fontSize: 9, color: T.muted, textTransform: 'uppercase', letterSpacing: 0.8, marginBottom: 2 }}>Available</div>
          <div style={{ fontSize: 18, fontWeight: 700, color: T.green }}>{available}</div>
        </div>
        <div style={{ flex: 1, background: '#fff', borderRadius: 8, padding: '8px 10px', border: `1px solid ${T.border}` }}>
          <div style={{ fontSize: 9, color: T.muted, textTransform: 'uppercase', letterSpacing: 0.8, marginBottom: 2 }}>Out</div>
          <div style={{ fontSize: 18, fontWeight: 700, color: T.orange }}>{checkedOut}</div>
        </div>
      </div>
